# # cnn/2_train_cnn.py
# # -------------------------------------------------------------------
# # Step 2: Train 1D-CNN on segmented EEG data
# # Вход: results/08_cnn_data/eeg_segments.npz
# # Выход: results/09_cnn_models/best_cnn_model.h5 + metrics
#
# import os
# import numpy as np
...
# OUTPUT_DIR = '../results/09_cnn_models'
# MODEL_FILE = os.path.join(OUTPUT_DIR, 'best_cnn_model.h5')
# SCALER_FILE = os.path.join(OUTPUT_DIR, 'scaler_cnn.pkl')
# METRICS_FILE = os.path.join(OUTPUT_DIR, 'cnn_metrics.json')
#
# # Параметры
# TEST_SIZE = 0.2
# RANDOM_STATE = 42
...
#
#     # Построение модели
#     model = build_model(input_shape=(window, n_channels))
#     model.summary()
#
#     # Callbacks
...
# cnn/2_train_cnn.py
# -------------------------------------------------------------------
# Step 2: Train 1D-CNN on segmented EEG data (TRAIN set) and evaluate on TEST set
# Вход: results/08_cnn_data/eeg_train_segments.npz
#       results/08_cnn_data/eeg_test_segments.npz
# Выход: results/09_cnn_models/best_cnn_model.h5 + metrics + scaler
...
# TEST_SIZE = 0.2
RANDOM_STATE = 42  # Можно использовать для воспроизводимости TF/Keras (см. ниже)
EPOCHS = 100
BATCH_SIZE = 32

# Установка сида для воспроизводимости (для TensorFlow/Keras)
# tf.random.set_seed(RANDOM_STATE)
# np.random.seed(RANDOM_STATE)
# import random
# random.seed(RANDOM_STATE)
# Можно добавить другие настройки для детерминированности, если это критично

def build_model(input_shape):
    model = Sequential([
        # padding='same' сохраняет размерность после свертки, полезно перед пулингом
        Conv1D(filters=32, kernel_size=3, activation='relu', padding='same', input_shape=input_shape),
        BatchNormalization(),  # Нормализация после активации или до? Чаще после Batch Norm, затем активация
...
def main():
    ...
    history = model.fit(
        ...
        validation_split=0.2,  # 20% обучающих данных пойдут на валидацию в процессе обучения
        epochs=EPOCHS,
        batch_size=BATCH_SIZE,
        callbacks=[es, mc]
    )
